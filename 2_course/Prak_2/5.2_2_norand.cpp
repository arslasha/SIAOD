...
void generate_text_file(const string& filename, int num_records) {
    ...
    int starting_id = 100000;  // Начальное значение номера зачетной книжки

    for (int i = 0; i < num_records; ++i) {
        int student_id = starting_id + i;  // Последовательные номера зачетных книжек
...
int main() {
    ...
    for (int num_records : record_counts) {
        ...
        int key_to_find = 100100;  // Пример ключа для поиска (в пределах сгенерированных номеров)
